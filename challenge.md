...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables?
	...
	*The Employee table is related to EmployeeTerritory one to one.*
	*EmployeeTerritory is many to one with Territory. The Territory*
	*table acts as a definitions table for EmployeeTerritory,*
	*A left join between EmployeeTerritory and Territory will*
	*let you print the territory name each employee is assigned to*

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

	*MongoDB is usefull when you have a very large quantity of data, and*
	*the robustness of a relational database isn't as important. For*
	*instance, searching social media posts. Consistancy is not as critical*
	*as something like financial records. This will make it more difficult*
	*to do some operations like sort or median (dependant on sort) that*
	*require the entire database structure to fit in main memory, but*
	*allows for the database to be spread horizontally over more less*
	*expensive hardware. Joining will also require more computationally*
	*intensive workarounds that structured vertically integrated databases*
	*make simple*

- What is "NewSQL", and what is it trying to achieve?

	*NewSQL is trying to combine the advantages of unstructured databases*
	*(particularly horizontal scalability) with the advantages of structured*
	*(relational) databases. It still needs to make compromises somewhere*
	*(consistency or availability), but are more robust than a classic*
